...
def EnforceConnectivity(L1: np.ndarray, L2: np.ndarray, a1: np.ndarray, a2: np.ndarray, b1: np.ndarray, b2: np.ndarray,
                        x1: np.ndarray, x2: np.ndarray, y1: np.ndarray, y2: np.ndarray, D1: np.ndarray, W: np.ndarray,
    ...
    for i in range(nRows):
        for j in range(nCols):
            if mask[i][j] == 0:
                ...
                centerD1[sLabel] += D1[i][j] * Weight  # elevation
                    ...
                    for m in range(minX, maxX + 1):
                        for n in range(minY, maxY + 1):
                            if not mask[m][n] and label[m * nCols + n] == L:
                                ...
                                centerD1[sLabel] += D1[m][n] * Weight  # elevation
                ...
                centerD1[sLabel] /= centerW[sLabel]  # elevation
    sLabel += 1
    Count = 0
    ...
    while len(Sarray) > 0:
        ...
        for I in range(len(Sarray[S].Neighbor)):
            D = (centerL1[Label1] - centerL1[Sarray[S].Neighbor[I]]) * (centerL1[Label1] - centerL1[Sarray[S].Neighbor[I]]) + \
                ...
                (centerD1[Label1] - centerD1[Sarray[S].Neighbor[I]]) * (centerD1[Label1] - centerD1[Sarray[S].Neighbor[I]])
        ...
        centerD1[Label2] = (W2 * centerD1[Label2] + W1 * centerD1[Label1]) / W  # elevation
        centerW[Label2] = W
