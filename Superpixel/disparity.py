...
def run(img_gray, disparity_map, name):
    ...
    SHOW_SAVE = 1
    superpixelNum = 100
    ratio = 0.45  # 空间位置(x,y)与颜色（l,a,b）的比值,值越大，越平滑，0.45作为0.4存在
    ratio_disparity = 0  # 视差值所占的比例
    img_gray_disparity = img_gray*(1-ratio_disparity) + disparity_map*ratio_disparity

    rows, cols = img_gray.shape
    img = np.zeros([rows, cols, 3], dtype="uint8")
    img[:, :, 0] = img_gray_disparity
    img[:, :, 1] = img_gray_disparity
    img[:, :, 2] = img_gray_disparity

    disparity = np.zeros([rows, cols, 3], dtype="uint8")
    disparity[:, :, 0] = disparity_map
    disparity[:, :, 1] = disparity_map
    disparity[:, :, 2] = disparity_map
    ...
    disparity_I = correlate(disparity.astype(np.float64),
                            gaus.reshape(gaus.shape[0], gaus.shape[1], 1),
                            mode="constant").round().astype(np.uint8)
    ...
    (L1, L2, a1, a2, b1, b2, x1, x2, y1, y2, D1, W,
     ...
     StepX, StepY, iterationNum, thresholdCoef, new_label) = Initialize(I, disparity_I, nCols, nRows, superpixelNum, ratio,
                                                                        ratio_disparity, label)
